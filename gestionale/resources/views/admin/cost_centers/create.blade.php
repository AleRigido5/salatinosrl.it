...
@section('title', 'Nuovo Centro di Costo')
...
<div class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">
                <i class="fas fa-plus-circle mr-2 text-lime-600"></i> Nuovo Centro di Costo
            </h1>
            <p class="text-gray-500 text-sm mt-1">Inserisci i dati del nuovo centro di costo</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow border border-gray-200">
        <form method="POST" action="{{ route('admin.cost_centers.store') }}" id="costCenterForm">
            @csrf
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="table_references" id="table_references" required
                            ...
                            <option value="ownership" {{ old('table_references') == 'ownership' ? 'selected' : '' }}>Proprietà</option>
                            <option value="entities" {{ old('table_references') == 'entities' ? 'selected' : '' }}>Cliente/Fornitore</option>
                        </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" 
                                   ...
                                   autocomplete="off">
                            <input type="hidden" name="id_references" id="id_references" value="{{ old('id_references') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="contrada" value="{{ old('contrada') }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="localita" value="{{ old('localita') }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                    ...
                    <div>
                        ...
                        <input type="text" name="coltura" value="{{ old('coltura') }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="foglio" value="{{ old('foglio') }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                               placeholder="es. 12">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="particella" value="{{ old('particella') }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                               placeholder="es. 345">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" name="superficie" value="{{ old('superficie', 0) }}" step="0.0001"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                               placeholder="0.0000">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="competenza" value="{{ old('competenza', 'Solare') }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" name="costo_h" value="{{ old('costo_h', 7.00) }}" step="0.50"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                               placeholder="7.00">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="num_h" value="{{ old('num_h', 8) }}" min="0" max="24"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                               placeholder="8">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="note" rows="3" 
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                              placeholder="Eventuali note aggiuntive...">{{ old('note') }}</textarea>
                </div>
                
                <!-- RIGA 8: Stato Attivo (full width) -->
                <div class="mb-4">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="valid" value="1" {{ old('valid', true) ? 'checked' : '' }}
            ...
            <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-gray-200">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i> Crea Centro di Costo
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    function searchReferences(searchTerm, type) {
        ...
        fetch(`/admin/api/references/search?type=${type}&q=${encodeURIComponent(searchTerm)}`)
            ...
            .then(data => {
                ...
                // Aggiungi event listener per ogni risultato
    ...
    if (initialType) {
        ...
        const oldValue = "{{ old('id_references') }}";
        if (oldValue) {
            // Se c'è un vecchio valore, carica il nome corrispondente
            fetch(`/admin/api/references/${initialType}/${oldValue}`)
                .then(response => response.json())
                .then(data => {
                    if (data.name) {
                        document.getElementById('reference_autocomplete').value = data.name;
                        document.getElementById('id_references').value = oldValue;
                    }
                });
        }
    }
