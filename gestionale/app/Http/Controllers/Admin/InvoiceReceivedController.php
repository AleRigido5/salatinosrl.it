...
class InvoiceReceivedController extends Controller
{
    ...
    public function edit(InvoiceReceived $invoice)
    {
        $ownerships = Ownership::where('valid', 1)->get();
        $entities = Entity::where('valid', 1)
            ->whereIn('entity_type', ['fornitore', 'entrambi'])
        ...
        return view('admin.invoices-received.edit', compact('invoice', 'ownerships', 'entities', 'costCenters'));
    }

    public function update(Request $request, InvoiceReceived $invoice)
    {
        $validated = $request->validate([
            'id_ownership' => 'nullable|exists:ownership,id_proprieta',
            'id_entities' => 'nullable|exists:entities,id_cliente',
        ...
        try {
            ...
            if (!empty($validated['rows'])) {
                foreach ($validated['rows'] as $row) {
                    ...
                    if (isset($row['id']) && $row['id']) {
                        ...
                        if ($invoiceRow) {
                            $invoiceRow->update($row);
                            $existingRowIds[] = $invoiceRow->id;
                        }
                    } else {
                        $newRow = InvoiceRow::create($row);
            ...
            DB::commit();
            return redirect()->route('admin.invoices-received.index')->with('success', 'Fattura aggiornata con successo.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy(InvoiceReceived $invoice)
    {
        try {
            $invoice->rows()->delete();
            $invoice->delete();
            return redirect()->route('admin.invoices-received.index')->with('success', 'Fattura eliminata con successo.');
        } catch (\Exception $e) {
