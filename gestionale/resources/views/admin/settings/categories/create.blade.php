            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <select name="category_id" id="category_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ $selectedCategory == $cat->id ? 'selected' : '' }} data-tabella="{{ $cat->tabella_riferimento }}">
                                {{ $cat->titolo }}
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('tabella_riferimento', $category->tabella_riferimento ?? '') }}"
                ...
                <div>
                    ...
                    <input type="text" name="valore" value="{{ old('valore') }}" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500">
                    @error('valore')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="valid" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500">
                            <option value="1" {{ old('valid', true) ? 'selected' : '' }}>Sì</option>
                            <option value="0" {{ !old('valid', true) ? 'selected' : '' }}>No</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t">
                    ...
                    <button type="submit" 
                        ...
                        <i class="fas fa-save mr-2"></i> Salva Impostazione
                    </button>
                </div>
            </div>
        </form>
